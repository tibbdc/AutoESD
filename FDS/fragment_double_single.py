...
def leftPrimerAttribute(ID,dict_left_primers,target_seq,dict_input_seq,screeningmarker_seq,config):
...
def rightPrimerAttribute(ID,dict_right_primers,target_seq,dict_input_seq,screeningmarker_seq,config):
    ...
    seq_dha=dha_left_primer_add_seq+dr_seq+dha_temp[:seq_dha_right_point+1]
...
def primers_submitted_output(dict_uha,dict_dha,dict_screening_marker,dict_verify2,ofile,dict_insert={}):
    ...
    ofile.write('test-' + dict_verify2['ID'] + '-1' + '\t'+ dict_verify2['PRIMER_LEFT_0_SEQUENCE'] + '\n')
    ofile.write('test-' + dict_verify2['ID'] + '-2' + '\t'+ dict_verify2['PRIMER_RIGHT_0_SEQUENCE'] + '\n')
...
#Design process
def design_process(input_file_path,screeningmarker_file_path,workdir,ref_genome,config,plasmid_file_path,screening_maker_removal=None):
    ...
    dict_primers_whole={'successful':{'uha':[],'dha':[],'verify2':[],'inserted':[],'screening_marker':[]},'failed':{'uha':[],'dha':[],'verify2':[],'inserted':[],'screening_marker':[]}}
    ...
    headers = ['seqid','ref','alt',"manipulation","primer-1","primer-2","primer-3","primer-4","primer-5","primer-6","primer-7","primer-8","test-primer-1","test-primer-2","test-primer-3","test-primer-4",]
    ...
    for key1 in dict_input_seq:
        ...
        if len(dictscreeningmarkerprimers)<10:
            ...
            dict_screening_marker_primers_attribute = verify2PrimerAttribute(key1,dictscreeningmarkerprimers,screeningmarker_seq)
            ...
            if len(dictleftprimers)<10:
                ...
                dict_left_primers_attribute = leftPrimerAttribute(key1,dictleftprimers,target_seq,dict_input_seq[key1],screeningmarker_seq,config)
                ...
                if len(dictrightprimers)<10:
                    ...
                    dict_right_primers_attribute = rightPrimerAttribute(key1,dictrightprimers,target_seq,dict_input_seq[key1],screeningmarker_seq,config)
                    ...
                    if dict_input_seq[key1]["level"] == ">90":
                        ...
                        if len(dictinsertedprimers)<10:
                            ...
                            if len(dictverify2primers)<10:
                                ...
                                primers_submitted_output(dict_left_primers_attribute,dict_right_primers_attribute,dict_screening_marker_primers_attribute,dict_verify2_primers_attribute,primer_order_fsave,dict_inserted_primers_attribute)
                    else:
                        if dict_input_seq[key1]['type']=="deletion" and screening_maker_removal=="No":
                            seq_verify2_temp = dict_left_primers_attribute['SEQ_VERIFY2_TEMP_UHA_HALF'] + screeningmarker_seq + dict_right_primers_attribute['SEQ_VERIFY2_TEMP_DHA_HALF']
                        else:
                            seq_verify2_temp = dict_left_primers_attribute['SEQ_VERIFY2_TEMP_UHA_HALF'] + dict_right_primers_attribute['SEQ_VERIFY2_TEMP_DHA_HALF']
                        dictverify2primers=primerDesign(key1,seq_verify2_temp,config,"verify_2")
                        if len(dictverify2primers)<10:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--plasmid', '-p', help='plasmid file', required=False)
    ...
    time1=time.time()
    design_process(input_file_path,screeningmarker_file_path,workdir,ref_genome,point_dict,plasmid_file_path,screening_maker_removal)
    time2=time.time()
    print(time2-time1)