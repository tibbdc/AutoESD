    ...
    if mutation_pos_index - left_arm_seq_length - max_verify_up_ponit < 0:
        ...
        if genome_ref.upper() == ref.upper():
            ...
            if len(alt)<=length_threshold_msddsc:
                info_dict["level"]="<=length_homologous_sequence"
            else:
                info_dict["level"]=">length_homologous_sequence"
            return info_dict
        else:
...
def generate_visualize_file(key1,dict_input_seq,screeningmarker_seq,config,dict_screeningmarker,dict_verify1,fsave,workdir,dict_verify2={},dict_insert={},screening_maker_removal=None):
    """
    Arguments:
        key1[str]: seq id
        dict_input_seq[dict]: {
...
def design_process(input_file_path,screeningmarker_file_path,workdir,ref_genome,config,assigned_tp1_seq=None,screening_maker_removal=None):
    ...
    headers = ['seqid','ref','alt',"manipulation","primer-5","primer-6","primer-7","primer-8","test-primer-1","test-primer-2","test-primer-3","test-primer-4","strand","geneid","mutation_pos_index","name",]
                    ...
                    if dict_input_seq[key1]['type']=="deletion" and screening_maker_removal=="No":
                        ...
                        if len(dictverify2primers)<10:
                            ...
                            generate_visualize_file(
                                ...
                                visualize_fsave,
                                workdir,
                                dict_verify2 = dict_verify2_primers_attribute,
                                screening_maker_removal =screening_maker_removal
                                )
