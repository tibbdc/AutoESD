...
def create_mutation_info(record,mutation_type,mutation_pos_index,ref,alt,strand,chrom,name,mun_id,config):
    ...
    if mutation_type == "deletion":
        ...
        genome_ref = record[mutation_pos_index:mutation_pos_index+len(ref)]
        if genome_ref.upper() == ref.upper():
            if len(alt)<=length_homologous_sequence:
                info_dict = {
                    "seq_uha_max_whole":str(record[
                        mutation_pos_index - max_left_arm_seq_length - max_verify_2_up_ponit:mutation_pos_index
                        ]),
                    ...
                    "level":"<=length_homologous_sequence",
                    "type":mutation_type,
                }
                return info_dict
            elif length_homologous_sequence<len(alt)<=length_threshold_msddsc:
                info_dict = {
                    "seq_uha_max_whole":str(record[
                        mutation_pos_index - max_left_arm_seq_length - max_verify_2_up_ponit :mutation_pos_index
                        ]),
                    ...
                    "level":">length_homologous_sequence&<length_threshold_msddsc",
                    "type":mutation_type,
                }
                return info_dict
            else:
                info_dict = {
                    ...
                    "level":">length_threshold_msddsc", # 增加一对引物
                    "type":mutation_type,
                }
                return info_dict
        else:
...
def leftPrimerAttribute(ID,dict_left_primers,target_seq,dict_input_seq,config,screeningmarker_seq=None):
    ...
    # P5前20bp
    dict_left_primers_attribute['PRIMER_LEFT_WHOLE_SEQUENCE']=uha_left_primer
    if screeningmarker_seq:
        # 处理screeningmarker_seq
        rev_uha_right_primer=uha_right_primer_rev+screeningmarker_seq[:length_homo_seq]
    else:
        if dict_input_seq["level"] == "del":
            # 处理deletion
            rev_uha_right_primer=uha_right_primer_rev+dict_input_seq["seq_dha_max_whole"][:length_homo_seq]
        elif dict_input_seq["level"] == ">length_homologous_sequence&<length_threshold_msddsc":
            # 处理insertion substitution的 20< alt <90
            rev_uha_right_primer=uha_right_primer_rev+dict_input_seq["seq_altered_add_uha"]
        elif dict_input_seq["level"] == ">length_threshold_msddsc":
            # 处理insertion substitution的 alt >90
            rev_uha_right_primer=uha_right_primer_rev+ dict_input_seq["seq_altered"][:length_homo_seq]
        else:
            # 处理deletion  和 insertion substitution的 alt <= 20
            rev_uha_right_primer=uha_right_primer_rev+dict_input_seq["seq_altered"]+dict_input_seq["seq_dha_max_whole"][:length_homo_seq-len(dict_input_seq["seq_altered"])]
    seq_uha=uha_temp[seq_uha_left_point_on_temp:seq_uha_right_point]+rev_uha_right_primer
    ...
    seq_verify_temp_uha_half= dict_input_seq['seq_uha_max_whole'][seq_uha_left_point_on_seq_uha_max_whole-max_verify_2_up_ponit:seq_uha_left_point_on_seq_uha_max_whole]+seq_uha_without_add
    dict_left_primers_attribute['SEQ_VERIFY_TEMP_UHA_HALF']=seq_verify_temp_uha_half
...
def rightPrimerAttribute(ID,dict_right_primers,target_seq,dict_input_seq,config,screeningmarker_seq=None):
    ...
    dha_temp=dict_input_seq['seq_dha_max_whole']
    seq_dha_right_point=dha_temp.find(dha_right_primer_rev)+len(dha_right_primer)-1
    dha_left_primer_add_seq = ""
    if screeningmarker_seq:
        # 处理screeningmarker_seq
        dha_left_primer_add_seq = screeningmarker_seq[-length_homo_seq:]
    else:
        if dict_input_seq["level"] == ">length_homologous_sequence&<length_threshold_msddsc":
            # 处理insertion substitution的 30< alt <90
            dha_left_primer_add_seq = dict_input_seq["seq_altered_add_dha"]
        elif dict_input_seq["level"] == ">length_threshold_msddsc":
            # 处理insertion substitution的 alt >90
            dha_left_primer_add_seq = dict_input_seq["seq_altered"][-length_homo_seq:]
    dict_right_primers_attribute['PRIMER_LEFT_WHOLE_SEQUENCE']=dha_left_primer_add_seq+dha_left_primer
    dict_right_primers_attribute['PRIMER_RIGHT_WHOLE_SEQUENCE']=dha_right_primer
    seq_dha=dha_left_primer_add_seq + dha_temp[:seq_dha_right_point+1]
    dict_right_primers_attribute['PRODUCT_SEQUENCE']=seq_dha
    dict_right_primers_attribute['PRODUCT_WHOLE_LENGTH']=len(seq_dha)
    seq_verify_temp_dha_half=dha_temp[:seq_dha_right_point+max_verify_2_down_ponit+1]
    dict_right_primers_attribute['SEQ_VERIFY_TEMP_DHA_HALF']=seq_verify_temp_dha_half
    dict_right_primers_attribute['SEQ_DHA_WITHOIUTE_ADD']=dha_temp[:seq_dha_right_point+1]
    return dict_right_primers_attribute

def insertPrimerAttribute(ID,dict_insert_primers,dict_input_seq,screeningmarker_seq=None):
    ...
    dict_insert_primers_attribute['PRIMER_LEFT_0_SEQUENCE']=insert_left_primer
    dict_insert_primers_attribute['PRIMER_RIGHT_0_SEQUENCE']=insert_right_primer
    ...
    if screeningmarker_seq:
        dict_insert_primers_attribute['PRODUCT_SEQUENCE']=screeningmarker_seq
        dict_insert_primers_attribute['PRIMER_PRODUCT_SIZE']=len(screeningmarker_seq)
    else:
        dict_insert_primers_attribute['PRODUCT_SEQUENCE']=dict_input_seq['seq_altered']
        dict_insert_primers_attribute['PRIMER_PRODUCT_SIZE']=len(dict_input_seq['seq_altered'])
...
def primers_submitted_output(dict_uha_screening_marker,dict_dha_screening_marker,dict_screening_marker,dict_verify1,ofile,dict_uha=None,dict_dha=None,dict_verify2=None,dict_insert={}):
    # with open(output_dir,"w") as ofile:
    ofile.write(dict_uha_screening_marker['ID'] + '-1' + '\t'+ dict_uha_screening_marker['PRIMER_LEFT_WHOLE_SEQUENCE'] + '\n')
    if dict_uha:
        ofile.write(dict_uha['ID'] + '-9' + '\t'+ dict_uha['PRIMER_RIGHT_WHOLE_SEQUENCE'] + '\n')
    ofile.write(dict_dha_screening_marker['ID'] + '-3' + '\t'+ dict_dha_screening_marker['PRIMER_RIGHT_WHOLE_SEQUENCE'] + '\n')
    if dict_dha:
        ofile.write(dict_dha['ID'] + '-10' + '\t'+ dict_dha['PRIMER_LEFT_WHOLE_SEQUENCE'] + '\n')
    ofile.write(dict_uha_screening_marker['ID'] + '-2' + '\t'+ dict_uha_screening_marker['PRIMER_RIGHT_WHOLE_SEQUENCE'] + '\n')
    ofile.write(dict_dha_screening_marker['ID'] + '-4' + '\t'+ dict_dha_screening_marker['PRIMER_LEFT_WHOLE_SEQUENCE'] + '\n')
    ...
    if dict_insert:
        ofile.write(dict_insert['ID'] + '-5' + '\t'+ dict_insert['PRIMER_LEFT_0_SEQUENCE'] + '\n')
        ofile.write(dict_insert['ID'] + '-6' + '\t'+ dict_insert['PRIMER_RIGHT_0_SEQUENCE'] + '\n')
...
def generate_visualize_file(key1,dict_input_seq,screeningmarker_seq,config,dict_uha_redesign,dict_dha_redesign,dict_screeningmarker,dict_verify1,fsave,workdir,dict_uha={},dict_dha={},dict_verify2={},dict_insert={},screening_maker_removal=None):
    ...
    Arguments:
        ...
        dict_uha_redesign[dict]: screeningmarker design information, uha right primer design with screeningmarker
        dict_dha_redesign[dict]: screeningmarker design information, dha left primer design with screeningmarker
    ...
    tmp["detail"] = "Assembled PCR product. The upstream homologous arm (UHA), the downstream homologous arm (DHA), screening marker and primer-1/2, primer-3/4, primer-7/8, are visualized." 
    ...
    ## 2.3 primer1
    primer1 = dict_uha_redesign['PRIMER_LEFT_WHOLE_SEQUENCE']
    tmp["params"].append(
        {
            "name":"primer-1",
            "start":str(ref_screeningmarker_seq.find(primer1)),
            "end":str(ref_screeningmarker_seq.find(primer1) + len(primer1)),
            'direction':1,
            'color':'orange',
        }
    )
    ...
    ## 2.3 primer2
    primer2 = dict_uha_redesign['PRIMER_RIGHT_WHOLE_SEQUENCE']
    primer2_rev = revComp(primer2)
    tmp["params"].append(
        {
            "name":"primer-2",
            "start":str(ref_screeningmarker_seq.find(primer2_rev)),
            "end":str(ref_screeningmarker_seq.find(primer2_rev) + len(primer2_rev)),
            'direction':-1,
            'color':'orange',
        }
    )
    ...
    ## 2.4 primer3
    primer3 = dict_dha_redesign['PRIMER_LEFT_WHOLE_SEQUENCE']
    tmp["params"].append(
        {
            "name":"primer-3",
            "start":str(ref_screeningmarker_seq.find(primer3)),
            "end":str(ref_screeningmarker_seq.find(primer3) + len(primer3)),
            'direction':1,
            'color':'blue',
        }
    )
    ...
    ## 2.4 primer4
    primer4 = dict_dha_redesign['PRIMER_RIGHT_WHOLE_SEQUENCE']
    primer4_rev = revComp(primer4)
    tmp["params"].append(
        {
            "name":"primer-4",
            "start":str(ref_screeningmarker_seq.find(primer4_rev)),
            "end":str(ref_screeningmarker_seq.find(primer4_rev) + len(primer4_rev)),
            'direction':-1,
            'color':'blue',
        }
    )
    ...
    tmp["params"].append(
        {
            "name":"test-primer-1",
            "start":str(ref_screeningmarker_seq.find(v1_primer1)),
            "end":str(ref_screeningmarker_seq.find(v1_primer1) + len(v1_primer1)),
            'direction':1,
            'color':'orange',
        }
    )
    ...
    tmp["params"].append(
        {
            "name":"test-primer-2",
            "start":str(ref_screeningmarker_seq.find(v1_primer2_rev)),
            "end":str(ref_screeningmarker_seq.find(v1_primer2_rev) + len(v1_primer2_rev)),
            'direction':-1,
            'color':'orange',
        }
    )
    ...
    linelist = [
        ...
        primer3,
        primer4,
        primer5 if not (screening_maker_removal == "No" and mutation_type == "deletion") else "",
        primer6 if not (screening_maker_removal == "No" and mutation_type == "deletion") else "",
        primer7,
        primer8,
        primer9 if not (screening_maker_removal == "No" and mutation_type == "deletion") else "",
        primer10 if not (screening_maker_removal == "No" and mutation_type == "deletion") else "",
        dict_verify1['PRIMER_LEFT_0_SEQUENCE'],
        dict_verify1['PRIMER_RIGHT_0_SEQUENCE'],
        dict_verify2['PRIMER_LEFT_0_SEQUENCE'] if not (screening_maker_removal == "No" and mutation_type == "deletion") else "",
        dict_verify2['PRIMER_RIGHT_0_SEQUENCE'] if not (screening_maker_removal == "No" and mutation_type == "deletion") else "",
        strand,
        geneid,
...
def design_process(input_file_path,screeningmarker_file_path,workdir,ref_genome,config,assigned_tp1_seq=None,screening_maker_removal=None):
    ...
    dict_primers_whole={'successful':{'uha':[],'dha':[],'uha_screening_marker':[],'dha_screening_marker':[],'verify1':[],'verify2':[],'inserted':[],'screening_marker':[]},'failed':{'uha':[],'dha':[],'verify1':[],'verify2':[],'inserted':[],'screening_marker':[]}}
    ...
    headers = ['seqid','ref','alt',"manipulation","primer-1","primer-2","primer-3","primer-4","primer-5","primer-6","primer-7","primer-8","primer-9","primer-10","test-primer-1","test-primer-2","test-primer-3","test-primer-4","strand","geneid","mutation_pos_index","name",]
    ...
    for key1 in dict_input_seq:
        ...
        if len(dictscreeningmarkerprimers)<10:
            ...
            dict_screening_marker_primers_attribute = insertPrimerAttribute(key1,dictscreeningmarkerprimers,dict_input_seq[key1],screeningmarker_seq)
            ...
            if len(dictleftprimers)<10:
                ...
                dict_left_screening_marker_primers_attribute = leftPrimerAttribute(key1,dictleftprimers,target_seq,dict_input_seq[key1],config,screeningmarker_seq)
                dict_primers_whole['successful']['uha_screening_marker'].append(dict_left_screening_marker_primers_attribute)
                ...
                if len(dictrightprimers)<10:
                    ...
                    dict_right_screening_marker_primers_attribute = rightPrimerAttribute(key1,dictrightprimers,target_seq,dict_input_seq[key1],config,screeningmarker_seq)
                    dict_primers_whole['successful']['dha_screening_marker'].append(dict_right_screening_marker_primers_attribute)
                    homo_arm_seq_1=dict_left_screening_marker_primers_attribute['SEQ_UHA_WITHOIUTE_ADD']
                    homo_arm_seq_2=dict_right_screening_marker_primers_attribute['SEQ_DHA_WITHOIUTE_ADD']
                    blast_input_file.write('>'+key1+'_UHA'+'\n'+homo_arm_seq_1+'\n')
                    blast_input_file.write('>'+key1+'_DHA'+'\n'+homo_arm_seq_2+'\n')
                    if dict_input_seq[key1]["level"] == ">length_threshold_msddsc":
                        ...
                        dict_right_primers_attribute = rightPrimerAttribute(key1,dictrightprimers,target_seq,dict_input_seq[key1],config)
                        ...
                        if len(dictinsertedprimers)<10:
                            ...
                            dict_inserted_primers_attribute = insertPrimerAttribute(key1,dictinsertedprimers,dict_input_seq[key1])
                            dict_primers_whole['successful']['inserted'].append(dict_inserted_primers_attribute)
                            seq_verify1_temp = screeningmarker_seq[-400:] + dict_right_primers_attribute['SEQ_VERIFY_TEMP_DHA_HALF']
                            ...
                            if len(dictverify1primers)<10:
                                ...
                                if assigned_tp1_seq:
                                    seq_verify1_temp_add_screeningmarker=screeningmarker_seq + dict_right_primers_attribute['SEQ_VERIFY_TEMP_DHA_HALF']
                                ...
                                seq_verify2_temp = dict_left_primers_attribute['SEQ_VERIFY_TEMP_UHA_HALF'] + dict_input_seq[key1]['seq_altered'] + dict_right_primers_attribute['SEQ_VERIFY_TEMP_DHA_HALF']
                        ...
                        if len(dictverify1primers)<10:
                            ...
                            if assigned_tp1_seq:
                                seq_verify1_temp_add_screeningmarker=screeningmarker_seq + dict_right_screening_marker_primers_attribute['SEQ_VERIFY_TEMP_DHA_HALF']
                                dict_verify1_primers_attribute = verify1PrimerAttribute(key1,dictverify1primers,seq_verify1_temp,seq_verify1_temp_add_screeningmarker,assigned_tp1_seq)
                            else:
                                dict_verify1_primers_attribute = verify1PrimerAttribute(key1,dictverify1primers,seq_verify1_temp)
                            dict_primers_whole['successful']['verify1'].append(dict_verify1_primers_attribute)
